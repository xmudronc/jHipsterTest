/* tslint:disable max-line-length */
import { ComponentFixture, TestBed, fakeAsync, tick } from '@angular/core/testing';
import { HttpResponse } from '@angular/common/http';
import { Observable, of } from 'rxjs';

import { JHipsterTestTestModule } from '../../../test.module';
import { UsrUpdateComponent } from 'app/entities/usr/usr-update.component';
import { UsrService } from 'app/entities/usr/usr.service';
import { Usr } from 'app/shared/model/usr.model';

describe('Component Tests', () => {
    describe('Usr Management Update Component', () => {
        let comp: UsrUpdateComponent;
        let fixture: ComponentFixture<UsrUpdateComponent>;
        let service: UsrService;

        beforeEach(() => {
            TestBed.configureTestingModule({
                imports: [JHipsterTestTestModule],
                declarations: [UsrUpdateComponent]
            })
                .overrideTemplate(UsrUpdateComponent, '')
                .compileComponents();

            fixture = TestBed.createComponent(UsrUpdateComponent);
            comp = fixture.componentInstance;
            service = fixture.debugElement.injector.get(UsrService);
        });

        describe('save', () => {
            it(
                'Should call update service on save for existing entity',
                fakeAsync(() => {
                    // GIVEN
                    const entity = new Usr(123);
                    spyOn(service, 'update').and.returnValue(of(new HttpResponse({ body: entity })));
                    comp.usr = entity;
                    // WHEN
                    comp.save();
                    tick(); // simulate async

                    // THEN
                    expect(service.update).toHaveBeenCalledWith(entity);
                    expect(comp.isSaving).toEqual(false);
                })
            );

            it(
                'Should call create service on save for new entity',
                fakeAsync(() => {
                    // GIVEN
                    const entity = new Usr();
                    spyOn(service, 'create').and.returnValue(of(new HttpResponse({ body: entity })));
                    comp.usr = entity;
                    // WHEN
                    comp.save();
                    tick(); // simulate async

                    // THEN
                    expect(service.create).toHaveBeenCalledWith(entity);
                    expect(comp.isSaving).toEqual(false);
                })
            );
        });
    });
});
